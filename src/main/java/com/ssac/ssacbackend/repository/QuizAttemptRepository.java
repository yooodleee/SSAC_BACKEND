...
public interface QuizAttemptRepository extends JpaRepository<QuizAttempt, Long> {
    ...
    List<QuizAttempt> findByGuestIdWithQuiz(@Param("guestId") String guestId);

    /**
     * guestId로 응시 기록 목록을 quiz 정보와 함께 페이지네이션으로 조회한다. Guest 기록 조회 API에 사용한다.
     */
    @Query(
        value = "SELECT qa FROM QuizAttempt qa JOIN FETCH qa.quiz WHERE qa.guestId = :guestId",
        countQuery = "SELECT COUNT(qa) FROM QuizAttempt qa WHERE qa.guestId = :guestId"
    )
    Page<QuizAttempt> findByGuestIdPagedWithQuiz(@Param("guestId") String guestId, Pageable pageable);
