...
□ 새로 추가하는 서비스 클래스에 테스트 파일이 함께 작성되는가?
  → 비즈니스 로직이 있는 서비스 클래스는 반드시 테스트 파일 동반
  → 없으면 ❌ 구조 충돌: "테스트 파일 없음 또는 excludes 등록 필요"

□ 신규 서비스 클래스가 excludes에 미등록 상태로 0%가 되는가?
  → jacocoTestCoverageVerification Rule 2 (클래스별 50%) 위반 가능
  → 테스트 미작성 시 반드시 아래 중 하나를 선택:
     A. 테스트 파일 작성 (비즈니스 로직 존재)
     B. build.gradle excludes 등록 + testing.md 제외 테이블 갱신 (로직 없음)
     C. excludes 등록 + ADR 작성 (추후 고도화 예정)

□ jacocoTestCoverageVerification Rule 2 (클래스별 50%) 통과 가능한 구조인가?
  → 새 서비스 클래스 추가 시 Rule 2 영향 범위 사전 확인 필수

□ 신규 Controller에 테스트 파일이 있는가?
  → 없으면 ❌ 구조 충돌: "Controller 테스트 파일 없음 또는 Rule 4 excludes 등록 필요"

□ Controller 테스트에 정상 케이스 및 서비스 호출 검증(verify)이 포함되는가?
  → new-feature.md STEP 8 Controller 레퍼런스 패턴 참고

□ jacocoTestCoverageVerification Rule 4 (Controller 클래스별 40%) 통과 가능한 구조인가?
  → 단순 위임(Swagger 더미 등) Controller는 excludes 등록 후 ADR 기록

### Redis 캐싱 구조 점검

SC에 Redis 캐싱이 포함된 경우 아래 항목을 추가로 점검한다:

□ @Cacheable 사용 시 직렬화 설정이 GenericJackson2JsonRedisSerializer인가?
  → ❌ 충돌: StringRedisTemplate 수동 캐싱으로 전환 필요
  → ADR-003 참고

□ 신규 캐시 키가 기존 키 형식과 일관성 있게 정의되어 있는가?
  → 기존 키 형식: "domain:type:{id}" (예: "contents:v4:list:...", "home:{userId}")
  → 불일치 시 ⚠️ 주의 항목

□ TTL이 명시적으로 설정되어 있는가?
  → 미설정 시 ❌ 충돌

□ 데이터 변경 API에 캐시 무효화 로직이 있는가?
  → 누락 시 ⚠️ 주의 항목
